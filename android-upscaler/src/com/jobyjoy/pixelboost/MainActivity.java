...
public class MainActivity extends Activity {
    ...
    private static final String GALLERY_FOLDER = "PixelBoost";
    ...
    private void buildUi() {
        // Edge-to-edge on every Android version (Android 15+ enforces it for apps targeting 35+):
        // we draw behind the system bars and pad the header / content with the insets ourselves.
        Window win = getWindow();
        win.setStatusBarColor(Color.TRANSPARENT);
        win.setNavigationBarColor(Build.VERSION.SDK_INT >= 26 ? 0xE6F4F6FA : 0x66000000);
        int ui = View.SYSTEM_UI_FLAG_LAYOUT_STABLE | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
                | View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION;
        if (Build.VERSION.SDK_INT >= 26) ui |= 0x10; // SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR (API 26)
        win.getDecorView().setSystemUiVisibility(ui);

        final LinearLayout screen = new LinearLayout(this);
        screen.setOrientation(LinearLayout.VERTICAL);
        screen.setBackgroundColor(0xFFF4F6FA);

        final LinearLayout header = new LinearLayout(this);
        header.setOrientation(LinearLayout.HORIZONTAL);
        header.setGravity(Gravity.CENTER_VERTICAL);
        header.setBackgroundColor(0xFF182038);
        ...
        about.setText("About");
        about.setTextColor(0xFFB9C8FF);
        about.setTextSize(15);
        about.setPadding(dp(12), dp(8), dp(4), dp(8));
        about.setContentDescription("About and licenses");
        ...
        screen.addView(header, new LinearLayout.LayoutParams(-1, -2));
        ...
        screen.setOnApplyWindowInsetsListener(new View.OnApplyWindowInsetsListener() {
            @Override
            public WindowInsets onApplyWindowInsets(View v, WindowInsets in) {
                int l = in.getSystemWindowInsetLeft(), r = in.getSystemWindowInsetRight();
                header.setPadding(l + dp(16), in.getSystemWindowInsetTop() + dp(12), r + dp(12), dp(12));
                scroll.setPadding(l, 0, r, in.getSystemWindowInsetBottom());
                return in.consumeSystemWindowInsets();
            }
        });
        scroll.addView(root);

        TextView intro = label("Make photos and drawings sharper and bigger: 2x, 4x, 4K or 8K. "
                + "Everything runs on your phone. Nothing is uploaded.", 14, false);
        ...
        TextView credit = label("Powered by Real-ESRGAN (BSD-3-Clause) and ONNX Runtime (MIT). "
                + "Long jobs: keep the app open until it finishes.", 11, false);
    ...
    private String queryName(Uri uri) {
        ...
        if ("image".equals(name) && uri.getLastPathSegment() != null) name = uri.getLastPathSegment();
    ...
    private void showAbout() {
        ...
        TextView t = new TextView(this);
        t.setPadding(dp(24), dp(16), dp(24), dp(8));
        t.setTextSize(14);
        t.setText(getString(R.string.app_name) + " " + version + "\n\n"
                + "Privacy: images are processed entirely on your device. The app has no internet access "
                + "and doesn't collect, store or share any personal data. Upscaled images are only saved "
                + "when you tap Save, to Pictures/" + GALLERY_FOLDER + ".\n\n"
                + "AI models: Real-ESRGAN by Xintao Wang, Liangbin Xie, Chao Dong and Ying Shan "
                + "(BSD-3-Clause). Inference: ONNX Runtime by Microsoft (MIT).\n\n"
    ...
    private void showLicenses() {
        String text;
        try {
            text = new String(readAsset("licenses.txt"), "UTF-8");
        } catch (Exception e) {
            text = "Could not load licenses: " + e;
        }
        TextView t = new TextView(this);
                ...
                try {
                    ...
                    ui.post(new Runnable() {
                        ...
                        public void run() {
                            running = false;
                            savedUri = uri;
                            status.setText("Saved to Pictures/" + GALLERY_FOLDER + "/" + name);
                            refreshButtons();
    ...
    private Uri writeToGallery(Bitmap bmp, String name, boolean png) throws Exception {
        ...
        if (Build.VERSION.SDK_INT >= 29) {
            v.put("relative_path", Environment.DIRECTORY_PICTURES + "/" + GALLERY_FOLDER);
            v.put("is_pending", 1);
            ...
            File dir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES), GALLERY_FOLDER);
